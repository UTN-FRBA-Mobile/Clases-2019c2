...
class EditImageFiltersFragment : Fragment(), ThumbnailsAdapter.ThumbnailsAdapterListener {
    ...
    private fun setImage(){
        ...
        processHandler.post {
            val image = viewModel.editedImage(context!!)
            activity?.runOnUiThread { image_to_edit.setImageBitmap(image) }
        }
    }

    private fun prepareThumbnail(view: View) {
        thumbnailItemList = ArrayList()
        mAdapter = ThumbnailsAdapter(activity!!, thumbnailItemList, 0,this)
        ...
        val r = Runnable {
            ...
            activity!!.runOnUiThread {
                if (viewModel.filterSelected.name != null){
                    mAdapter.setSelectedFilter(viewModel.filterSelected.name)
                }
                mAdapter.notifyItemRangeInserted(0, thumbnailItemList.size - 1)
            }
        }
